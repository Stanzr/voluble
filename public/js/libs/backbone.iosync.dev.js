(function (undefined) {
  ...
  var evtLogger = logger('backbone iosync:');
...
Backbone.sync = function (method, model, options) {
  ...
  var cmd = getUrl(model).split('/'), namespace = (cmd[0] !== '') ? cmd[0] : cmd[1]; // if leading slash, ignore
  evtLogger('sendRequest: '+namespace+":"+method);
  var params = _.extend({
  ...
  evtLogger('emitting '+namespace+':'+method);
  io.emit(namespace + ':' + method, params.data, function (err, data) {
    if (err) {
      options.error(err);
    } else {
      evtLogger('get data '+namespace+':'+method);
